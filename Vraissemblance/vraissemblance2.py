...
####################### 
sigma_d2 = np.random.uniform(1,2)
sigma_s2 = np.random.uniform(1,2)
beta_d = np.random.uniform(-5,5, size = 8)
beta_s = np.random.uniform(-5,5, size = 8)
...
i = 10
def int_denominateur(t):
    deno1 = 1 - (1 + sigma_d2 * phi_d[i] * IDD((seller['tau_end'][i] - seller['tau_birth'][i]), t, delta, alpha_d)) ** ( - sigma_d2 - 1)
    deno2 = phi_s[i] * lambdaS(alpha_s, t) * (1 + sigma_s2 * phi_s[i] * IS(t, alpha_s)) ** ( - sigma_d2 - 1)
    return deno1 * deno2
print(quad(int_denominateur, 0, 1000)[0])
