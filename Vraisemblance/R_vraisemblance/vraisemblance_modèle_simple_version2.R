...
# Création du dataframe à 954 x 2 lignes : 
seller_clone <- data.frame(matrix(ncol = ncol(seller), nrow = 0))

# Parcourez chaque ligne du dataframe initial et ajoutez-la deux fois
for (i in 1:nrow(seller)) {
  ligne <- seller[i, ]
  seller_clone <- rbind(seller_clone, ligne)
  seller_clone <- rbind(seller_clone, ligne)
}
rownames(seller_clone) <- NULL

# Je change le nom du dataframe pour ne pas avoir à modifier toutes mes lignes de code ... 
# L'ancien dataframe "seller" devient "ex_seller"
ex_seller <- seller
seller <- seller_clone 
...
log_likelihood <- function(lambda_d, lambda_s, delta, beta_d_1, beta_d_2, beta_d_3, beta_d_4, beta_d_5, beta_d_6,
  ...
  L_seller_sum <- 0
  L_clone_sum <- 0
  for (i in 1:nrow(ex_seller)) {
    # Lorsque i est impair (1, 3, 5 ...) (resp. pair) nous avons à faire à un seller (resp. clone)
    indice_seller = 2 * i - 1  # 1, 3, ...
    indice_clone = 2 * i # 2, 4, ...
    Log_seller_i <- log(LSeller_i(lambda_d, lambda_s, phi_d, phi_s, delta, indice_seller))
    Log_clone_i <- log(LClone_i(lambda_d, lambda_s, phi_d, phi_s, delta, indice_clone))
    L_seller_sum <- L_seller_sum + Log_seller_i
    L_clone_sum <- L_clone_sum + Log_clone_i
  }
  Likelihood <- (L_seller_sum + L_clone_sum) / nrow(seller)  # 1908
  return(-Likelihood)}
