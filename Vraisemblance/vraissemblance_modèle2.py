...
def LSeller_i(alpha_d, alpha_s, sigma_d2, sigma_s2, phi_d, phi_s,delta, i):
    ...
    denominateur = quad(integrande_denominateur, 0, 1000)[0]
    #resultat
...
def LClone_i(alpha_d, alpha_s, sigma_d2, sigma_s2, phi_d, phi_s,delta, i):
    ...
    def integrande_denominateur(t):
        gauche = 1 - (1 + sigma_d2 * phi_d[i] * IDD(alpha_d, alpha_s, delta, t_end, t)) ** (- sigma_d2 - 1)
        droite = phi_s[i] * lambda_s(alpha_s, t) * (1 + sigma_s2 * phi_s[i] * IS(alpha_s, t)) ** (- sigma_s2 - 1)
        return gauche * droite
    denominateur = quad(integrande_denominateur, 0, 1000)[0]
    #resultat
    return numerateur / denominateur
...
initial_params = [random.uniform(1, 2), random.uniform(1, 2), random.uniform(1, 2), random.uniform(1, 2), 10.79241345, 
