...
### Nomenclature ###
X = c('type_libre','sexe_homme','idf','etranger','une_tete','dec1','dec2','dec3')
...
beta_d <- parametres[3:956]  # 954 éléments pour beta_d
beta_s <- parametres[957:1910]  # 954 éléments pour beta_s
delta <- parametres[1911]
...
for (i in donnees$index) {
    L_seller_i <- LSeller_i(lambda_d, lambda_s, phi_d, phi_s, delta, donnees, i)
    L_clone_i <- LClone_i(lambda_d, lambda_s, phi_d, phi_s, delta, donnees, i)
    L_seller_vector <- c(L_seller_vector, log(L_seller_i))
    L_clone_vector <- c(L_clone_vector, log(L_clone_i))}
...
beta_d0 <- runif(954, min = 0, max = 1)
beta_s0 <- runif(954, min = 0, max = 1)
